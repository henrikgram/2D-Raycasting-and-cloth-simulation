...
namespace Glsl
{
    ...
    /// pointA translation by (x, y) looks as follows:
    /// \code
    /// float array[9] =
    /// {
    ///     1, 0, 0,
    ...
    /// elements, aligned in column-major order. For example,
    /// pointA translation by (x, y, z) looks as follows:
    /// \code
    /// float array[16] =
    /// {
    ///     1, 0, 0, 0,
...
///
/// All vector types support pointA default constructor that
/// initializes every component to zero, in addition to pointA
/// constructor with one parameter for each component.
/// The components are stored in member variables called
/// x, y, z, and w.
///
/// All matrix types support pointA constructor with pointA float*
/// parameter that points to pointA float array of the appropriate
/// size (that is, 9 in pointA 3x3 matrix, 16 in pointA 4x4 matrix).
